...
describe LoanAlerts::PriorityGrouping do
  ...
  describe "#groups_hash" do
    Timecop.freeze(Date.parse("26-10-2012")) do
      let!(:high_priority_loan1) { FactoryGirl.create(:loan, :offered, maturity_date: 59.weekdays_ago(Date.today)) }

      let!(:high_priority_loan2) { FactoryGirl.create(:loan, :offered, maturity_date: 50.weekdays_ago(Date.today)) }

      let!(:medium_priority_loan1) { FactoryGirl.create(:loan, :offered, maturity_date: 49.weekdays_ago(Date.today)) }

      let!(:medium_priority_loan2) { FactoryGirl.create(:loan, :offered, maturity_date: 30.weekdays_ago(Date.today)) }

      let!(:low_priority_loan1) { FactoryGirl.create(:loan, :offered, maturity_date: 29.weekdays_ago(Date.today)) }

      let!(:low_priority_loan2) { FactoryGirl.create(:loan, :offered, maturity_date: Date.today) }
      ...
      let(:priority_grouping) {
        ...
        LoanAlerts::PriorityGrouping.new(loans_array, 59.weekdays_ago(Date.today).to_date, Date.today, :maturity_date)
      }

...
describe 'loan recovery' do
  let(:loan) { FactoryGirl.create(:loan, :settled, dti_demand_outstanding: '123', dti_amount_claimed: '456', settled_on: '1/5/12') }
  let(:current_user) { FactoryGirl.create(:lender_user, lender: loan.lender) }
  before { login_as(current_user, scope: :user) }

  [Loan::Settled, Loan::Recovered, Loan::Realised].each do |state|
    context "with state #{state}" do
      let(:loan) { FactoryGirl.create(:loan, state.to_sym, dti_demand_outstanding: '123', dti_amount_claimed: '456', settled_on: '1/5/12') }
